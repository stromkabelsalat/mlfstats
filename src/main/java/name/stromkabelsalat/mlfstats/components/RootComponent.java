...
public class RootComponent {
	...
	@Value("${name.stromkabelsalat.mlfstats.category-ids}")
	...
	public Root root() {
		...
		for(final List<Mlf2Entry> mlf2EntryListByCategory: mlf2EntryListByCategoryList) {
			final Integer categoryId = mlf2EntryListByCategory
					.get(0)
					.getCategory();
			final String categoryName = this
					.mlf2CategoryRepository
					.findById(categoryId)
			...
			final Integer startExclusiveYear = mlf2EntryListByCategory.get(mlf2EntryListByCategory.size() - 1).getTime().getYear() + 1;
			...
			for (final Integer yearNumber : yearNumbers) {
				...
				for(final Integer monthNumber: monthNumbers) {
					...
					final LocalDateTime endTime = startTime.plusMonths(1).minusSeconds(1);

					final List<Mlf2Entry> mlf2EntriesByCategoryInMonth = this.mlf2EntryRepository.findByCategoryAndTimeBetweenOrderById(categoryId, startTime, endTime);

					if(!mlf2EntriesByCategoryInMonth.isEmpty()) {
						final Mlf2Entry firstEntry = mlf2EntriesByCategoryInMonth.get(0);
						final Mlf2Entry lastEntry = mlf2EntriesByCategoryInMonth.get(mlf2EntriesByCategoryInMonth.size() -1);
